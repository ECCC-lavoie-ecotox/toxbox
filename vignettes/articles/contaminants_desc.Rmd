...
## Objectif 1: Otenir une liste exhaustive des contaminants mesurés chez quatre espèces impliquées dans le PASL
...
## Objectif 2: Déterminer les tailles d'échantillons pour les combinaisons: Site, espèce, années, contaminant
...
#### Criteria 1. On isole les contaminants qui ont été mesuré une seule fois par année pour un site.
...
#### Criteria 2. On retire les sites pour lesquelles, il y a une seule année de collecte (impossibilité de calibrer un modèle de tendance temporelle).
...
```{R}
data_count <- data_count |>
    dplyr::filter(!Location %in% crit2) |>
    dplyr::ungroup()
```

#### Figures: Taille des échantillons par site 
...
for (s in seq_len(nrow(combinaisons))[1:2]) {

    d <- data_count |> 
        dplyr::filter(conpound_family == combinaisons$conpound_family[s] & Location == combinaisons$Location[s]) |>
        dplyr::mutate(Year = as.character(Year))
    
    cat("\n##### ", combinaisons$conpound_family[s], "sur le site", combinaisons$Location[s],  "\n")

    p <- ggplot(d, aes(x = Year, y = variable, fill = n)) + 
        geom_tile(color = 'white') + 
        coord_fixed() + 
        viridis::scale_fill_viridis(trans = 'reverse') + 
        scale_x_discrete(breaks = seq(1969, 2023, 1)) +
        theme_minimal() +
        theme(
